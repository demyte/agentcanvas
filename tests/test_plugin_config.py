...
class PluginConfigTests(unittest.TestCase):
    ...
    def test_skill_documents_cli_contract_and_blank_stub_contract(self) -> None:
        ...
        self.assertIn("Use `export-html` to create or intentionally regenerate", skill)
        self.assertIn("it can overwrite the custom UI with the blank starter shell", skill)
        self.assertIn("run `open -id <canvas-id>` and use the returned `http://127.0.0.1` URL", skill)
        self.assertIn("Prefer the Canvas HTTP server over `file://` paths", skill)
        self.assertIn("127.0.0.1:12345", skill)
        self.assertIn("open -id review-board", skill)
        self.assertIn("The shared starter template intentionally has no `<body>` content", skill)
        self.assertIn("needs a new or materially revised browser UI", skill)
        ...
        self.assertIn("Thread identifiers are associations, not anchors", skill)
