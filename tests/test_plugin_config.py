...
class PluginConfigTests(unittest.TestCase):
    ...
    def test_skill_documents_cli_contract_and_blank_stub_contract(self) -> None:
        ...
        self.assertIn("Treat `export-html` as a starter/regeneration command", skill)
        self.assertIn("It can overwrite an existing customized `canvas.html`", skill)
        self.assertIn("Use `open -id <canvas-id>` and reload the returned HTTP URL", skill)
        self.assertIn("prefer the local Canvas HTTP server over opening `file://` paths", skill)
