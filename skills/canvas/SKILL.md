...
When presenting a canvas after any create, update, refresh, validation, inspection, or other canvas work, include the current `http://127.0.0.1` canvas URL. Also include: canvas id, scope and anchor, storage path, files created or updated, promotion status, and how to continue. Be explicit when something is only a working artifact.
